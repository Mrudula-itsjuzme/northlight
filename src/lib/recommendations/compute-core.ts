...
/**
 * Role-free core: gathers real signals, ranks them, and replaces the
 * brand's full recommendation set inside a transaction. Split out of the
 * `"use server"` action (computeRecommendations in actions.ts) so it can
 * also be called directly by the background job worker (Phase 12,
 * `compute_recommendations` job type), which runs outside an
 * authenticated request context — same core/action split as
 * persistGapReportsForCompetitor, runPipeline, generateContentBrief, and
 * processDocument.
 */
export async function computeAndPersistRecommendations(brandId: string): Promise<{ count: number }> {
  const db = getDb();
