...
export async function getDiagnosticsSummary(brandId?: string): Promise<DiagnosticsSummary> {
  try {
    ...
    const rawAvg = evalStats?.avgOverall;
    const avgOverallScore = typeof rawAvg === "number" && !Number.isNaN(rawAvg) ? rawAvg : 0;
    ...
    const recTotal = recStats?.total || 0;
    const recAccepted = recStats?.accepted || 0;
    const acceptanceRatePct = recTotal > 0 ? (recAccepted / recTotal) * 100 : 0;

    // 7. Active Unresolved Visibility alerts count
    const alertCondition = brandId
      ? and(eq(visibilityAlerts.brandId, brandId), eq(visibilityAlerts.isResolved, false))
      : eq(visibilityAlerts.isResolved, false);
    ...
    return {
      executionMode: getExecutionMode(),
      openAiKeyConfigured: Boolean(process.env.OPENAI_API_KEY),
      workerQueue: { queued: 0, running: 0, succeeded: 0, failed: 0 },
      llmTelemetry: { totalTokens: 0, totalCostCents: 0, stepCount: 0 },
      usageEventsCount: 0,
      cacheMetrics: { totalHits: 0, tokensSaved: 0, costSavedCents: 0 },
      evaluations: { avgOverallScore: 0, totalEvaluations: 0 },
      recommendations: { acceptanceRatePct: 0, totalFeedbackCount: 0 },
      visibilityAlertsCount: 0,
    };
  }
}
